...
describe("BaseErc20Example",  () => {
    let contract;
    let accounts, owner;
    const name = 'BaseERC20';
    const symbol = 'BERC20';
    const decimals = 18;
    let totalSupply  ; // 100 million
    let randomAddr ;
    async function init() {
        totalSupply  = BigInt(100000000) * (10n ** BigInt(decimals));
        const randomAccount = ethers.Wallet.createRandom();
        randomAddr= randomAccount.address;
        // 部署 BaseERC20
    ...
    describe("transfer", function () {
        it("Should fail if sender doesn’t have enough balance", async function () {
            ...
            console.log("currentBalance:",curBalance);
